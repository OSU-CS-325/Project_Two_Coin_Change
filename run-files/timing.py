...
def Q3():
	...
	plt.figure(1)
	plt.subplot(2, 1, 1)
	plt.plot(change, runtimeGreedy, 'b.', linewidth=2.0, label='Greedy data')
	plt.plot(change, fitGreedy, 'k--', linewidth=2.0, label='Curve Fit')
	plt.legend(loc='upper left')
	plt.ylabel('Avg. Runtime (sec)')
	plt.title('V = [1, 5, 10, 25, 50]')
	plt.grid(True)
	plt.subplot(2, 1, 2)
	plt.plot(change, runtimeDP, 'r.', linewidth=2.0, label='DP data')
	plt.plot(change, fitDP, 'k--', linewidth=2.0, label='Curve Fit')
	plt.legend(loc='upper left')
	plt.ylabel('Avg. Runtime (sec)')
